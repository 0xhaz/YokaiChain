...
import {IYokaiChainDescriptor} from "src/interfaces/IYokaiChainDescriptor.sol";
...
contract YokaiChainDescriptorMock is IYokaiChainDescriptor {
    ...
    uint256[] internal EARRINGS_ITEMS =
        [90100, 81600, 73100, 64600, 56100, 48100, 40100, 33100, 26100, 19100, 12100, 8100, 4100, 200, 0];
    uint256[] internal ACCESSORY_ITEMS =
        [88000, 79500, 71500, 64000, 56500, 49500, 42500, 35750, 29000, 22500, 16000, 10000, 4000, 300, 100, 0];
    ...
    uint256[] internal HAIR_ITEMS =
        [90350, 82100, 74100, 66350, 59100, 52100, 45350, 39100, 33100, 27350, 22100, 17100, 12350, 8100, 4100, 200, 0];
        ...
        returns (NFTDescriptor.SVGParams memory)
    {
        ...
        return NFTDescriptor.SVGParams({
            hair: detail.hair,
            eye: detail.eye,
    ...
    function getBackgroundId(NFTDescriptor.SVGParams memory params) private view returns (uint8) {
        uint256 score = itemScoreProba(params.hair, HAIR_ITEMS) + itemScoreProba(params.accessory, ACCESSORY_ITEMS)
            + itemScoreProba(params.earring, EARRINGS_ITEMS) + itemScoreProba(params.mask, MASK_ITEMS)
            + itemScoreProba(params.mouth, MOUTH_ITEMS) + itemScoreProba(params.skin, SKIN_ITEMS)
            + itemScoreProba(params.nose, NOSE_ITEMS) + itemScoreProba(params.mark, MARK_ITEMS)
            + itemScoreProba(params.eye, EYE_ITEMS) + itemScoreProba(params.eyebrow, EYEBROW_ITEMS);
    ...
    function getScore(NFTDescriptor.SVGParams memory params) external view returns (uint256) {
        return itemScoreProba(params.hair, HAIR_ITEMS) + itemScoreProba(params.accessory, ACCESSORY_ITEMS)
            + itemScoreProba(params.earring, EARRINGS_ITEMS) + itemScoreProba(params.mask, MASK_ITEMS)
            + itemScoreProba(params.mouth, MOUTH_ITEMS) + itemScoreProba(params.skin, SKIN_ITEMS)
            + itemScoreProba(params.nose, NOSE_ITEMS) + itemScoreProba(params.mark, MARK_ITEMS)
            + itemScoreProba(params.eye, EYE_ITEMS) + itemScoreProba(params.eyebrow, EYEBROW_ITEMS);
    ...
    /// @dev Get the score of an item based on its probability
    ...
    /// @dev Multiply the raw score by a factor
